...
2.  Start a Moby development image.
    ...
    ```bash
    $ docker run --privileged --rm -ti -v `pwd`:/go/src/github.com/Prakhar-Agarwal-byte/moby docker-dev:dry-run-test /bin/bash
    ```
...
```bash
$ TESTDIRS='github.com/Prakhar-Agarwal-byte/moby/opts' make test-unit
```
...
```bash
$ TESTDIRS='github.com/Prakhar-Agarwal-byte/moby/opts' TESTFLAGS='-test.run ^TestValidateIPAddress$' make test-unit
```
...
2.  Change to the `moby` source directory.
    ...
    ```bash
    $ cd /c/gopath/src/github.com/Prakhar-Agarwal-byte/moby
    ```
