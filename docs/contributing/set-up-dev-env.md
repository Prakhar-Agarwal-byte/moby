...
your Docker container.
   ...
   > **Note**: Inspecting the `Dockerfile` shows a `COPY . /go/src/github.com/Prakhar-Agarwal-byte/moby` instruction, suggesting that dynamic code changes will _not_ be reflected in the container. However inspecting the `Makefile` shows that the current working directory _will_ be mounted via a `-v` volume mount.
