package fakestorage // import "github.com/Prakhar-Agarwal-byte/moby/testutil/fakestorage"
...
func ensureHTTPServerImage(t testing.TB) {
	...
	if _, err = os.Stat("../contrib/httpserver/httpserver"); os.IsNotExist(err) {
		...
		cmd := exec.Command(goCmd, "build", "-o", filepath.Join(tmp, "httpserver"), "github.com/Prakhar-Agarwal-byte/moby/contrib/httpserver")
